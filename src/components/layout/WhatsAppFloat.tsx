...
export default function WhatsAppFloat() {
  return (
    <a
      ...
      aria-label="Chat with Raju ji on WhatsApp"
      title="Chat with R³S Realty on WhatsApp"
      className="fixed bottom-6 right-6 z-50 group"
    >
      <div className="relative w-14 h-14 bg-bg border border-gold/60 text-gold hover:bg-gold hover:text-bg flex items-center justify-center transition-all pulse-ring">
        <WhatsAppIcon className="w-6 h-6" />
      </div>
      <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 hidden md:group-hover:block bg-bg border border-border-strong px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] text-text whitespace-nowrap">
        Chat with Raju ji
      </span>
    </a>
  );
